...
public class UDSQL_REP_DETAIL_T2_DAO {
	...
	public int saveSQL(String e, String f) {
		String query = "insert into cdwmeta.UDSQL_REP_DETAIL_T3 values('" + e + "','" + f + "')";
		return jdbcTemplate.update(query);
	}

	public void saveSQLWithSP(String e, String f) {
		SimpleJdbcCall simpleJdbcCall = new SimpleJdbcCall(jdbcTemplate).withSchemaName("CDWMETA").withProcedureName("insert_parsed_sql");

		Map<String, Object> inParamMap = new HashMap<String, Object>();
		inParamMap.put("P_UNID", e);
		inParamMap.put("P_SQL", f);
	...
	public List<UDSQL_REP_DETAIL_T2> findAll() {
		...
		String sql = "SELECT cast(UNID as varchar2(100)) as UNID,replace(replace(SQL_TEXT,'promptmany',''),'#','') as SQL_TEXT FROM CDWMETA.UDSQL_REP_DETAIL_T2 where sql_text is not null and round(length(sql_text)/1000,0)>100";
